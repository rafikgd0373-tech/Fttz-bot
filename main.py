...
async def on_message(message):
    ...
    if leveled:
        await message.channel.send(
            f"🎉 مبروك {message.author.mention}! وصلت للفل **{user['level']}**! 🚀\n🎉 Congrats {message.author.mention}! You reached level **{user['level']}**! 🚀"
        )
...
async def leaderboard(ctx):
    ...
    embed = discord.Embed(title="🏆 أغنى الأعضاء | Richest Members - m00d", color=0xFFD700)
...
async def buy(ctx, item_id: str):
    ...
    if not item:
        await ctx.send("❌ العنصر غير موجود! | Item not found!\n\nاكتب `!shop` لتشوف الأشياء | Type `!shop` to see items.")
        return
    ...
    if user["mood"] < item["price"]:
        ...
        await ctx.send(f"❌ ما عندكش فلوس كافية! | Not enough money!\n\nتحتاج **{needed:,} m00d** أكثر | You need **{needed:,} m00d** more.")
        return
    ...
    embed = discord.Embed(
        title="✅ تم الشراء! | Purchase Successful!",
        description=f"اشتريت **{item['name']}** بـ **{item['price']:,} m00d**\nYou bought **{item['name']}** for **{item['price']:,} m00d**!\n\n💵 رصيدك | Balance: **{user['mood']:,} m00d**",
        color=0x00FF88
    )
    await ctx.send(embed=embed)
...
async def inventory(ctx, member: discord.Member = None):
    ...
    if not inv:
        await ctx.send(f"🎒 مخزن {member.display_name} فاضي! | {member.display_name}'s inventory is empty!")
        return

    embed = discord.Embed(title=f"🎒 مخزن | Inventory: {member.display_name}", color=0x8A5CFF)
    item_counts = {}
    for iid in inv:
